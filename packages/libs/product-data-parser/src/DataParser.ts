...
import { normalizeWhitespaces } from '@package/utils';
import { priceToNumber } from '@package/money';
...
export class DataParser {
  ...
  parse(type: ProductType, content: string, featureMapObject: TFeatureMapObject = {}) {
    ...
    const getElementAttr = (element: cheerio.Cheerio, selector: string, attrName: string) =>
      (element.find(selector).attr(attrName) || '').trim();
