...
import { enumToArrKeys } from '@react-and-express/utils';
import { fetchContents, readDataFromFile, writeDataToFile } from '@react-and-express/product-data-io';
import { mergeData, DataParser, TFeatureMapObject } from '@react-and-express/product-data-parser';
...
export const updateDataController = async (req: Request, res: Response) => {
  ...
  const parseNewContents = (productType: ProductType, baseUrl: string, contents: (string | null)[], featureMapObject: TFeatureMapObject = {}) => {
    return contents
  ...
  if (productType === ProductType.DISKS) {
    newData.push(...parseNewContents(productType, amazonUkBaseUrl, await fetchContents([
    ...
    newData.push(...parseNewContents(productType, amazonUkBaseUrl, await fetchContents([
      `${amazonUkBaseUrl}/hz/wishlist/genericItemsPage/P2ZKOL4X451E?${urlQueryParams}`,
    ]), {
      size: sizeRegExp,
      color: colorRegExp,
    }))
    ...
    newData.push(...parseNewContents(productType, amazonDeBaseUrl, await fetchContents([
      `${amazonDeBaseUrl}/hz/wishlist/ls/19MQHNQNMJSXW?${urlQueryParams}`,
    ])))
  }
